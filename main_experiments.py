...
class MyLogger(flw.Logger):
    def log(self, server=None):
        ...
        # dataset = server['task']
        if not os.path.exists('results_experiments'.format(server.option['task'])):
            os.mkdir('results_experiments'.format(server.option['task']))

        if not os.path.exists('results_experiments/{}'.format(server.option['task'])):
            os.mkdir('results_experiments/{}'.format(server.option['task']))

        if not os.path.exists('results_experiments/{}/add_{}_delete_{}'.format(server.option['task'],
                                                                                server.option['num_clients_add'],
                                                                                server.option['num_clients_delete'])):
            os.mkdir('results_experiments/{}/add_{}_delete_{}'.format(server.option['task'], 
                                                                      server.option['num_clients_add'],
                                                                        server.option['num_clients_delete']))

        test_results_path = 'results_experiments/{}/add_{}_delete_{}/test_results.csv'.format(server.option['task'], 
                                                                                server.option['num_clients_add'],
                                                                                server.option['num_clients_delete'])
        ...
        for client in server.all_clients:
            ...
            client_save_path = f'results_experiments/{task_name}/add_{num_add}_delete_{num_delete}/{client_name}.csv'
