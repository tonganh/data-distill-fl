...
class BasicEdge(BasicClient):
    ...
    def collect_distilled_data_from_client(self, clients: List[BasicClient]):
        ...
        for client in clients:
            print(f"collecting client {client.name}")
...
class BasicMobileClient(BasicClient):
    ...
    def distill_data(self):
        ...
        self.x_distill, self.y_distill = self.distiller.distill(x_train, y_train, x_val, y_val, additional_message=message)
    
    def load_distill_data(self):
        self.x_distill = torch.load(os.path.join(self.distill_save_path,'_x_distill.pt')).detach().cpu().numpy()
        self.y_distill = torch.load(os.path.join(self.distill_save_path,'_y_distill.pt')).detach().cpu().numpy()
